...
def clear_chat_history():
    del st.session_state["messages_chat"]
    if 'messages_chat' not in st.session_state:
        st.session_state['messages_chat'] = [{"role": "assistant", "content": lc.gt("use-case-ass-first-reply")}]

#USAGE BUTTON RESET HISTORY
st.button(lc.gt("use-case-button-forget"), on_click=clear_chat_history, type="primary", key=2020)

# Store LLM generated responses
if "messages_chat" not in st.session_state.keys():
    st.session_state.messages_chat = [{"role": "assistant", "content": lc.gt("use-case-ass-first-reply")}]
...
if user_prompt := st.chat_input():
    st.session_state.messages_chat.append({"role": "user", "content": user_prompt})
    with st.chat_message("user"):
        st.write(user_prompt)

# Generate a new response if last message is not from assistant
if st.session_state.messages_chat[-1]["role"] != "assistant":
    ...
    st.session_state.messages_chat.append(message)
